...
### Added
- `linear-workflow` CLI binary (`npm link` / `npm exec linear-workflow`) wrapping `scripts/diagnose.mjs` so the documented `linear-workflow config diagnose` command is real.
...
### Changed
- `isDoneEligible` is now aware of the active Profile's `completion_gate`: strict (`production_deployment`) can no longer be bypassed by a user release confirmation; `manual` requires explicit manual confirmation; `release_confirmed` accepts user confirmation or trusted deployment evidence.
- Config parser now fails closed on malformed input: requires `version: 1`, requires `profile`, requires `overrides` to be an object, rejects unknown top-level keys, and throws on unparseable YAML lines instead of silently defaulting to standard.
