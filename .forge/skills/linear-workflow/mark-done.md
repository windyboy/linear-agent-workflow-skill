...
1. 验证 identifier 为完整边界格式 `\b[A-Z]{1,5}-\d+\b`（比较时可规范大小写）；格式错误或不存在时单独报告。
2. 逐个读取 issue，记录标题、原状态、team、assignee、priority 与发布证据。
3. 已 completed 的 issue 跳过；canceled/triage 不改；backlog/unstarted 不改并说明需先走生命周期。仅 started/Review 等可完成状态进入写入。
