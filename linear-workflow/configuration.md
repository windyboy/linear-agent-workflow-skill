...
Optional `linear-workflow.config.yaml` in project root. Default profile: `standard`.
...
- `disabled` — no local files; Layer 1 Binding still created for new issues
...
v0.2.0 behavior ≈ `profile: strict`. Create config file and adjust profile/overrides as needed.
...
| Unexpected behavior | Effective config, team/project scope, overrides spelling |
