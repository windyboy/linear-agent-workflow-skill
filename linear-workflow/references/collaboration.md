...
Optional coordination for a personal or small-project Issue. It adds a few bounded local workstreams; it does not change the Linear lifecycle, profiles, gates, or the five invariants.

## When to use it

Use only when a Coordinator needs to split one Issue into a small number of bounded tasks or ask for an independent cross-check. Without a task packet, use the normal single-Agent flow unchanged.

Create the first packet during planning. That opts the Issue into collaboration-lite locally, but does **not** change its Linear state. Dispatching a Worker is execution and happens only after the normal authorized `started` transition.

## Roles and authority

| Role | Does | Must not do |
|---|---|---|
| Coordinator | Creates packets, accepts an owner, compares evidence, and writes authorized Linear summaries | Substitute for the user or bypass Invariant 3 |
| Worker | Completes one packet and appends its result to `report.md` | Write Linear or expand the declared scope |
...
Only the Coordinator writes Linear comments, bindings, descriptions, or state. This is an Invariant 3 authorization refinement, not a sixth invariant: every Linear write still needs its normal authorization, read-before-write, and read-back verification.
...
One Issue has at most a few workstreams. A packet declares one owner session, bounded files or symbols, acceptance criteria, and optional dependencies. If two sessions request the same packet, the Coordinator records the first accepted owner and resolves the duplicate manually. There are no claims, leases, heartbeats, TTLs, or durable ledger.

Use these paths within the existing Execution Context root when Layer 2 is enabled:

```text
.agent-work/<issue>/workstreams/<workstream-id>/task-packet.md
.agent-work/<issue>/workstreams/<workstream-id>/report.md
.agent-work/<issue>/workstreams/<workstream-id>/review.md
```
