...
- Code completion or passing CI is not user verification. Move to In Review according to the active Review Gate policy (see `linear-workflow/configuration.md`): either when the user explicitly says they verified it, or when PR is ready and CI passes (depending on profile).sed”).
